...
class AbstractSZZ(ABC):
        ...
        if not os.path.isdir(self._repository_path):
            if repos_dir:
                ...
                if os.path.isdir(repo_dir):
                    log.info(f"Local-cloning repository {repo_full_name} (hardlinks)...")
                    Repo.clone_from(url=repo_dir, to_path=self._repository_path, multi_options=['--local'])
                else:
    ...
    def __cleanup_repo(self):
        ...
        def _on_rm_error(func, path, exc_info):
            # On Windows, .git pack files are read-only; clear the bit and retry.
            try:
        ...
        if os.path.isdir(self.__temp_dir):
            rmtree(self.__temp_dir, onerror=_on_rm_error)
